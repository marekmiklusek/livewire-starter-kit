...
<body class="bg-black text-white antialiased" x-data="{ sidebarOpen: true }">
    ...
    <div class="min-h-screen flex">
        <!-- Sidebar -->
        <aside
            class="fixed inset-y-0 left-0 z-50 flex flex-col bg-neutral-900 border-r border-neutral-800 transition-all duration-300 overflow-hidden"
        ...
        >
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                <a
                    ...
                    wire:navigate
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('dashboard') ? 'bg-neutral-800 text-amber-400' : 'text-neutral-400 hover:bg-neutral-800 hover:text-white' }}"
                >
                ...
                <a
                    href="{{ route('sample') }}"
                    wire:navigate
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('sample') ? 'bg-neutral-800 text-amber-400' : 'text-neutral-400 hover:bg-neutral-800 hover:text-white' }}"
                >
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="pt-4 mt-4 border-t border-neutral-800">
                    ...
                    <a
                        href="{{ route('profile') }}"
                        wire:navigate
                        class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all {{ request()->routeIs('profile') ? 'bg-neutral-800 text-amber-400' : 'text-neutral-400 hover:bg-neutral-800 hover:text-white' }}"
                    >
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="mt-1">
                        ...
                        <button
                            type="submit"
                            class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-neutral-400 hover:bg-red-500/10 hover:text-red-400 transition-all"
                        >
                            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-3 border-t border-neutral-800">
                <div class="flex items-center gap-3 px-3 py-2 rounded-xl bg-neutral-800">
